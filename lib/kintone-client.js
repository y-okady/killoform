...
/**
 * アプリのアクション設定を取得（動作テスト環境）
 */
async function getAppActions(appId, baseUrl, apiToken) {
  const client = createClient(baseUrl, apiToken);
  try {
    const result = await client.app.getAppActions({ app: appId, preview: true });
    return result.actions;
  } catch (error) {
    throw new Error(`アプリアクション設定の取得に失敗しました: ${error.message}`);
  }
}
 ...
 * アプリアクション設定を更新
 */
async function updateAppActions(appId, baseUrl, apiToken, actions, revision) {
  const client = createClient(baseUrl, apiToken);
  try {
    const result = await client.app.updateAppActions({
      app: appId,
      actions: actions,
      revision: revision,
    });
    ...
    throw new Error(`アプリアクション設定の更新に失敗しました: ${error.message}`);
  }
}
