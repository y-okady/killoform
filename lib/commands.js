...
async function planCommand(appId) {
  ...
  if (!localFields && !localLayout && !localViews && !localReports && !localPlugins && !localPluginConfigs) {
  ...
  // フォームフィールドの差分
  if (localFields) {
    const hasChanges = printDiff(currentFields, localFields, `apps/${appId}/form/fields.json`);
    if (!hasChanges) {
      console.log(`\napps/${appId}/form/fields.json: 変更なし`);
    } else {
      hasAnyChanges = true;
    }
  } else {
  ...
  // レイアウトの差分
  if (localLayout) {
    const hasChanges = printDiff(currentLayout, localLayout, `apps/${appId}/form/layout.json`);
    if (!hasChanges) {
      console.log(`\napps/${appId}/form/layout.json: 変更なし`);
    } else {
      hasAnyChanges = true;
    }
  } else {
  ...
  // 一覧の差分
  if (localViews) {
    const hasChanges = printDiff(currentViews, localViews, `apps/${appId}/views.json`);
    if (!hasChanges) {
      console.log(`\napps/${appId}/views.json: 変更なし`);
    } else {
      hasAnyChanges = true;
    }
  } else {
  ...
  // レポートの差分
  if (localReports) {
    const hasChanges = printDiff(currentReports, localReports, `apps/${appId}/reports.json`);
    if (!hasChanges) {
      console.log(`\napps/${appId}/reports.json: 変更なし`);
    } else {
      hasAnyChanges = true;
    }
  } else {
  ...
  // プラグイン情報の差分
  if (localPlugins) {
    const hasChanges = printDiff(plugins, localPlugins, `apps/${appId}/plugin.json`);
    if (!hasChanges) {
      console.log(`\napps/${appId}/plugin.json: 変更なし`);
    } else {
      hasAnyChanges = true;
    }
  } else {
  ...
  if (localPluginConfigs) {
    const hasChanges = printDiff(currentPluginConfigs, localPluginConfigs, `apps/${appId}/plugin/config.json`);
    if (!hasChanges) {
      console.log(`\napps/${appId}/plugin/config.json: 変更なし`);
    } else {
      hasAnyChanges = true;
    }
  } else {
...
async function copyCommand(sourceAppId, targetAppId) {
  ...
  const targetDir = path.join(process.cwd(), "apps", targetAppId);
